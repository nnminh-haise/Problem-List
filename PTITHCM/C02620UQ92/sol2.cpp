...
void update(int index, int L, int R, int position, int value) {
    if (position < L and R < position) return;
    if (L == R) {
...
int get_sum(int index, int L, int R, int U, int V) {
    if (!(L <= U and V <= R)) return(0);
    if (L == R) return (segment_tree[index]);
    int mid = (L + R) / 2;
    return (get_sum(2 * index, L, mid, U, V) + get_sum(2 * index + 1, mid + 1, R, U, V));
}
